...
#define IFPRINT(arg) if(data_i==138 && total_changes==-12) (arg)
...
// above, is that the first lower limit is always -Inf and the last
// upper limit is always Inf! This is so we can test if there is any
// significant increase in the number of intervals.
void PeakSegPDPAInf
(int *data_vec, double *weight_vec, int data_count,
 int maxSegments,
 // the following matrices are for output:
 ...
 int *intervals_mat){//maxSegments x maxSegments(model up to last data point).
  ...
  for(int data_i=0; data_i<data_count; data_i++){
    ...
    PiecewisePoissonLossLog *cost_model = &cost_model_vec[data_i];
    cost_model->piece_list.emplace_back
      (1.0, -data_weight_cumsum/weight_cumsum, 0.0, -INFINITY, INFINITY, -1, false);
  }
